...
export function CapitalCouncil({ era, selectedInstitution, setSelectedInstitution, officials, onSelectOfficial, onCourt, onNorthZhili, open, setOpen, mobile = false }: {
  ...
  const selectedName = selectedPeriod?.display_name || selectedInstitution.name;
  const [showTemplate, setShowTemplate] = useState(false);
  const [showCabinetCatalog, setShowCabinetCatalog] = useState(false);
  const useTemplate = selectedPeriod?.use_current_office_template !== false || showTemplate;
  const occupiedIds=new Set(records.filter(record=>record.record_kind!=='event').flatMap(record=>record.office_ids||[]));
  const temporalOfficials = officials.filter(official=>selectedInstitution.id!=='cabinet'||showCabinetCatalog||occupiedIds.has(official.record_id)).map(official=>{
    const name = official.title_periods?.find(item=>item.from<=year&&item.until>=year)?.name;
  ...
  const roster=scopeIds ? officialsByIds(scopeIds,officials) : officials.filter(official=>selectedInstitution.keywords.some(word=>`${official.institution} ${official.department||''} ${official.title}`.includes(word)));
  ...
  const select = (institution: CapitalInstitution, branch: string|null = null) => {
    setSelectedInstitution(institution); setBranchId(branch); setQuery(''); setShowHistory(false); setShowTemplate(false); setShowCabinetCatalog(false); setOpen(true);
  };
  ...
  return <section className={mobile ? 'phone-council' : 'council-board animate-rise-in'}>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent showCloseButton={false} className="institution-floating-card"><DialogClose className="floating-close" aria-label="关闭官署浮窗"><X size={22}/></DialogClose>
      ...
      <div className="institution-floating-scroll" ref={rosterScroll}>
        ...
        {selectedInstitution.id==='cabinet'&&<div className="history-period-note"><p>年度图显示本年已对应的阁衔；阁衔不是必须满员的六个固定席位。未明确所带殿阁衔的阁臣仍列在上方人物中。</p><button className="history-text-button" onClick={()=>setShowCabinetCatalog(value=>!value)}>{showCabinetCatalog?'回到本年已载阁衔':'查看其他殿阁职制资料'}</button></div>}
        {selectedPeriod?.use_current_office_template===false && <div className="history-period-note"><p>{selectedPeriod.note}</p><button className="history-text-button" onClick={()=>setShowTemplate(v=>!v)}>{showTemplate?'收起后期职制参照':'查看明中后期职制参照'}</button></div>}
        ...
        {useTemplate && <OfficialHierarchy mobile={mobile} institutionName={selectedName} ministry={selectedMinistry} selectedBranch={selectedBranch} central={selectedCentral} officials={temporalOfficials} fallbackOfficials={roster} onSelect={viewOfficial} onBranch={selectBranch} showHistory={showHistory} query={query.trim()}/>}
